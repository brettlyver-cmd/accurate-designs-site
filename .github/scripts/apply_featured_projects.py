...
if count != 1:
    raise SystemExit(f"Expected to replace one PROJECTS block, replaced {count}")

pinfo = '''function PInfo({ p }) {
  return (
...
if count != 1:
    raise SystemExit(f"Expected to replace one PInfo block, replaced {count}")

old_home = '<p style={{ ...sn, fontSize: 12, fontWeight: 400, lineHeight: 1.6, color: "rgba(245,241,235,0.3)" }}>{p.desc}</p>'
new_home = '<p style={{ ...sn, fontSize: 12, fontWeight: 400, lineHeight: 1.6, color: "rgba(245,241,235,0.3)" }}>{p.challenge}</p>'
if old_home not in text:
    raise SystemExit("Expected Featured Projects summary paragraph was not found")
...
for item in required:
    if item not in text:
        raise SystemExit(f"Verification failed: {item}")

path.write_text(text, encoding="utf-8")
print("Featured Projects correction applied successfully.")
